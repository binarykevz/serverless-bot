...
export class KevinAI {
  ...
  private async initializeSchema(): Promise<void> {
    ...
    await this.db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_conversations_user_seq ON conversations (user_id, seq DESC)`,
      args: [],
    });
  }
  ...
  private ensureSchema(): Promise<void> {
    ...
    if (!this.schemaPromise) {
      this.schemaPromise = this.initializeSchema().catch(console.error);
    }
  ...
  async generateText(prompt: string): Promise<string> {
    if (!this.geminiKey) {
      return "Wait lang, may problema sa connection ko.";
    }
    ...
    try {
      ...
      if (!res.ok) {
        return "Hmm, medyo mabagal connection ko ngayon.";
      }

      const data = await res.json();
      return parseGeminiText(data) || "Hmm.";
    } catch {
  ...
  async describeImage(
    ...
    try {
      ...
      const prompt =
        customPrompt ||
        "Look at this image. Give a very short, natural, and sweet reaction (1 sentence max). Act like a normal person. Do not use the crying emoji. Do not sound like an AI.";
      ...
      const data = await res.json();
      return parseGeminiText(data) || "Got this for you 🤍";
    } catch {
      return "Got this for you 🤍";
    }
  }
  ...
  async getHistory(userId: string, limit = 10): Promise<HistoryMessage[]> {
    ...
    try {
      ...
      const result = await this.db.execute({
        sql: "SELECT role, content FROM conversations WHERE user_id = ? ORDER BY seq DESC LIMIT ?",
        args: [userId, limit],
      });
